...
internal class ZepbenTokenFetcherTest {
    ...
    @Test
    fun testFetchAuth0TokenSuccessfulUsingCredentials() {
        validateFetchTokenSuccessfulUsingCredentials(AuthMethod.AUTH0, "audience=test_audience")
    }

    @Test
    fun testFetchEntraTokenSuccessfulUsingCredentials() {
        validateFetchTokenSuccessfulUsingCredentials(AuthMethod.ENTRAID, "scope=test_audience/.default")
    }
    ...
    fun testFetchAuth0TokenSuccessfulUsingRefresh() {
        fetchTokenSuccessfulUsingRefresh(AuthMethod.AUTH0, "audience=test_audience")
    }

    @Test
    fun testFetchEntraTokenSuccessfulUsingRefresh() {
        fetchTokenSuccessfulUsingRefresh(AuthMethod.ENTRAID, "scope=test_audience/.default")
    }
    ...
    private fun validateFetchTokenSuccessfulUsingCredentials(authMethod: AuthMethod, expectedAudienceArgument: String) {
        validateFetchTokenSuccessful(authMethod, expectedAudienceArgument, refreshToken = null)
    }

    private fun fetchTokenSuccessfulUsingRefresh(authMethod: AuthMethod, expectedAudienceArgument: String) {
        validateFetchTokenSuccessful(authMethod, expectedAudienceArgument, "test_refresh_token")
    }

    private fun validateFetchTokenSuccessful(authMethod: AuthMethod, expectedAudienceArgument: String, refreshToken: String?) {
        doReturn(StatusCode.OK.code).`when`(response).statusCode()
        doReturn(
        ...
        mockStatic(BodyPublishers::class.java, CALLS_REAL_METHODS).use { bodyPublishers ->
            ...
            verify(client).send(any(), any<HttpResponse.BodyHandler<String>>())
            bodyPublishers.verify { BodyPublishers.ofString(matches("$expectedAudienceArgument${refreshToken?.let {"&refresh_token=$it" } ?: ""}")) }
